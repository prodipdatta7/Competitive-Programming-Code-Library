...
/**
     ...
     *
     *  created : Tuesday 11-May, 2021  08:34:36 PM
**/
...
vector < vector < int > > g ; 
int col[N], cntColor[3] ;

int dfs(int source, int choosen, int parent = 0) {
    col[source] = col[parent] == 1 ? 2 : 1 ; 
    cntColor[col[source]]++ ; 
    for(int i : g[source]) {
        if(i == parent)continue ; 
        if(choosen)dfs(i, 0, source) ; 
    }
}
...
int main() {
    ...
    while (test--) {
        ...
        dfs(1);
        cout << min(cntColor[1], cntColor[2]) << '\n' ; 
    }
