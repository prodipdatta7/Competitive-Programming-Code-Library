/**
     ...
     *  author  :   Prodip Datta
     *  handle  :   prodipdatta7
     *  created :   Wednesday 02-September, 2020  01:42:45 PM
**/
...
int main() {
    ...
    while (test--) {
        ...
        for (int i = 0 ; i < n ; ++i) {
            a[i].ff = upper_bound(all(val), a[i].ff) - val.begin() ;
            a[i].ss = upper_bound(all(val), a[i].ss) - val.begin() ;
        }
        FenwickTree var(n + n) ;
        int r = 0, i = 0, cnt = 0 ;
        while (i < n) {
            int cur = 1 + var.query(a[i].ff) ;
            cmax(cnt, cur) ;
            var.update(a[i].ss, cur) ;
            ++i ; 
        }
        cout << cnt << '\n' ;
    }
