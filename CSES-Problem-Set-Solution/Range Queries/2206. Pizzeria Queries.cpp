...
/**
     ...
     *
     *  created : Wednesday 24-February, 2021  05:43:56 PM
**/
...
struct Node {
	...
	int sum, diff ;
	Node(int _sum = inf, int _diff = inf): sum(_sum), diff(_diff) {}

	Node mergeNodes(Node other) {
		return Node(min(sum, other.sum), min(diff, other.diff)) ;
	}
} ;
	...
	while (test--) {
		...
		while(q--) {
			...
			if(tp == 2){
				...
				Node R = query(1, 1, n, x + 1, n) ; 
				cout << min(L.diff + x, R.sum - x) << '\n' ; 
			}
			else {
				Int(y) ; 
				update(1, 1, n, x, y) ; 
