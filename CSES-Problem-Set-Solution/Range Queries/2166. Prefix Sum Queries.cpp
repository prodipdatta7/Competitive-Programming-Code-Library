...
/**
     ...
     *
     *  created : Wednesday 24-February, 2021  05:06:45 PM
**/
...
struct Node {
	...
	Node(ll _maxSum = -Inf, ll _totalSum = 0): maxSum(_maxSum), totalSum(_totalSum) {}

	Node mergeNodes(Node other) {
		Node cur(-Inf, totalSum + other.totalSum) ;
		cmax(cur.maxSum, maxSum) ;
		cmax(cur.maxSum, totalSum + other.maxSum) ;
		cmax(cur.maxSum, cur.totalSum) ;
		return cur ;
	}
...
Node query(int i, int b, int e, int l, int r) {
	...
	int mid = b + e >> 1 ;
	return query(i << 1, b, mid, l, r).mergeNodes(query(i << 1 | 1, mid + 1, e, l, r)) ;
}
...
int main() {
	...
	while (test--) {
		...
		while(q--){
			...
			int l, r ; 
			Int(tp, l, r) ; 
			if(tp == 2)cout << max(0LL,query(1, 1, n, l, r).maxSum) << '\n' ;
