/**
     ...
     *  author  :   Prodip Datta
     *  handle  :   prodipdatta7
     *  created :   Monday 31-August, 2020  04:46:17 PM
**/
...
void NearestMinToLeft(vector < int > &a, vector < int > &dp, int n) {
    stack < int > st ;
    for (int i = 1 ; i <= n ; ++i) {
        while (sz(st) > 0 and a[st.top()] >= a[i])
            st.pop() ;
        if (sz(st) > 0)dp[i] = st.top() ;
        st.push(i) ;
    }
}

void NearestMinToRight(vector < int > &a, vector < int > &dp, int n) {
    stack < int > st ;
    for (int i = n ; i >= 1 ; --i) {
        while (sz(st) > 0 and a[st.top()] >= a[i])
            st.pop() ;
...
int main() {
    ...
    while (test--) {
        ...
        vector < int > L(n + 2, 0), R(n + 3, n + 1) ; 
        NearestMinToRight(a, R, n) ; 
        NearestMinToLeft(a, L, n) ; 
        long long res = 0 ;
        for(int i = 1 ; i <= n ; ++i){
            int len = R[i] - L[i] - 1 ;
            res = max(res, 1LL * len * a[i]) ; 
        }
        cout << res << '\n' ; 
    }
