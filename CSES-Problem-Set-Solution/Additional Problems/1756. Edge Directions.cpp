/**
     ...
     *  author  :   Prodip Datta
     *  handle  :   prodipdatta7
     *  created :   Monday 31-August, 2020  03:47:10 PM
**/
...
void dfs(int s, int p, vector < pii > &edges) {
    label[s] = 1 + label[p] ;
    for (pii i : g[s]) {
        if (!label[i.ff]) {
            dfs(i.ff, s, edges) ;
            if (edges[i.ss].ff != s)
                swap(edges[i.ss].ff, edges[i.ss].ss) ;
        }
        else {
            if (label[i.ff] < label[s]) {
                if (edges[i.ss].ff != i.ff)
                    swap(edges[i.ss].ff, edges[i.ss].ss) ;
            }
        }
    }
