 ...
 *  author  :   Prodip Datta
 *  handle  :   prodipdatta7
 *  created :   Monday 31-August, 2020  03:38:34 PM
**/
...
int main() {
    ...
    while (test--) {
        ...
        for (int mask = 1 ; mask < (1 << f) ; ++mask) {
            /*here mask starts from 1 to avoid the empty subset */
            long long sum = 0 ;
            for (int i = 0 ; i < f ; ++i) {
                if (mask >> i & 1)sum += b[i] ;
            }
        ...
        sort(all(Rsum)) ;
        long long res = 0 ;
        res += (upper_bound(all(Lsum), S) - lower_bound(all(Lsum), S)) ;
        if (f) {
            for (long long i : Lsum) {
                res += (upper_bound(all(Rsum), S - i) - lower_bound(all(Rsum), S - i)) ;
            }
        }
        cout << res << '\n' ;
    }
