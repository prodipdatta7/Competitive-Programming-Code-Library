/**
     ...
     *  author  :   Prodip Datta
     *  handle  :   prodipdatta7
     *  created :   Wednesday 29-July, 2020  10:28:49 AM
**/
...
template < typename T, typename T1 > inline T modpow(T b, T1 p) {
    T res = 1 ;
    if (b >= mod)b %= mod ;
    while (p > 0) {
...
int main() {
    ...
    while (test--) {
    	...
    	for(int i = 1 ; 1LL * i * i <= n ; ++i){
    		int j = (n / i) % mod ; 
    		res = add(res, mul(mul(add(i, j, mod), (j - i + 1), mod), modInv, mod), mod) ; 
    		res = add(res, mul(i,  (j - i), mod), mod) ; 
    	}
    	cout << res << '\n' ; 
    }
