/**
     ...
     *  author  :   Prodip Datta
     *  handle  :   prodipdatta7
     *  created :   Tuesday 28-July, 2020  12:49:37 PM
**/
...
int dp[N][101], a[N], n, m ;

int call(int ind, int v) {
	if (ind == 0)return 1 ;
	int& ret = dp[ind][v] ;
	if (~ret)return ret ;
	ret = 0 ;
	if (a[ind] != 0) {
		if (abs(v - a[ind]) > 1)return 0 ;
		ret = call(ind - 1, a[ind]) ;
	}
	else {
		if (ind == n) {
			for (int i = 1 ; i <= m ; ++i)
				ret = add(ret, call(ind - 1, i), mod) ; 
			}
		else {
			ret = add(ret, call(ind - 1, v), mod) ;
			if (v - 1 >= 1)ret = add(ret, call(ind - 1, v - 1), mod) ;
			if (v + 1 <= m)ret = add(ret, call(ind - 1, v + 1), mod) ;
		}
	}
	return ret ;
}
